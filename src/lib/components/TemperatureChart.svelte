<script>
  ...
  // Register core Chart.js components (using TimeScale instead of CategoryScale)
  if (typeof window !== 'undefined') {
        ...
        onClick: (event, activeElements, chart) => {
          if ((timeframe === 'week' || timeframe === 'month') && activeElements.length > 0) {
            ...
            if (dayIndices.length > 0) {
              ...
              const dailyTemps = data.datasets[0].data.slice(
                dayIndices[0],
                dayIndices[dayIndices.length - 1] + 1
              );
              const dailyMax = Math.max(...dailyTemps);
              const dailyMin = Math.min(...dailyTemps);
              dayAnnotation = {
                type: 'box',
                xMin,
                xMax,
