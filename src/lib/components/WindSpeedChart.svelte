      ...
      ? {
          ...
          onClick: (event, activeElements, chart) => {
            if ((timeframe === 'week' || timeframe === 'month') && activeElements.length > 0) {
              ...
              // data.labels are assumed to be ISO date strings or timestamps
              ...
              if (dayIndices.length > 0) {
                ...
                const dailyWinds = data.datasets[0].data.slice(
                  dayIndices[0],
                  dayIndices[dayIndices.length - 1] + 1
                );
                const dailyMax = Math.max(...dailyWinds);
                const dailyMin = Math.min(...dailyWinds);
                dayAnnotation = {
                  type: 'box',
                  xMin,
                  xMax,
